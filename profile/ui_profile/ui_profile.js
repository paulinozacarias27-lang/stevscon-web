const MessagesSystem = {
    ...
    async sendMessage(convId, text) {
        ...
        // Validación básica también en esta capa.
        const trimmed = (text || '').trim();
        if (!trimmed) return;
        try {
            await ProfileMemory.sendMessage(convId, current.uid, trimmed);
        } catch (e) {
    ...
    async renderMessagesPage() {
        if (typeof UIMessages === 'undefined' || !UIMessages.renderMessagesPage) {
            console.warn('Módulo UIMessages no cargado.');
            return;
        }
        const current = ProfileSystem.getCurrentProfile();
        if (!current) return;
    ...
    async renderConversation(convId) {
        ...
        // MEJORA B: intentamos obtener datos frescos del otro usuario; si falla,
        // usamos la copia almacenada en la conversación como respaldo.
        let otherProfile = otherUid ? (conv.participantProfiles && conv.participantProfiles[otherUid]) : null;
        if (otherUid) {
            const freshProfile = await ProfileMemory.getProfile(otherUid);
            if (freshProfile) otherProfile = freshProfile;
        }
        ...
        // MEJORA B: forzamos scroll al último mensaje al abrir la conversación.
        if (UIMessages.requestScrollToBottom) UIMessages.requestScrollToBottom();
