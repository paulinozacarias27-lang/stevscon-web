const UIProfile = {
    ...
    renderEditModal(profile) {
        ...
        // MEJORA E: se agrega la clase profile-edit-overlay para animar la aparicion del modal (fade-in).
        ...
        overlay.innerHTML = `
            <div class="stevscon-accounts-card profile-edit-card" style="max-width:500px;width:100%;margin:auto;position:relative;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="edit-username" value="${esc(profile.username || '')}" maxlength="32" oninput="UIProfile._validateUsername()">
                    <div id="username-feedback" style="font-size:0.75rem;color:var(--text-muted);margin-top:4px;"></div>
                </div>
    ...
    // MEJORA D: validacion del nombre de usuario en tiempo real (feedback inmediato bajo el campo).
    _validateUsername() {
        ...
        if (value.length < 2) {
            feedback.textContent = 'Debe tener al menos 2 caracteres.';
            feedback.style.color = 'var(--danger)';
            return false;
        }
        if (value.length > 32) {
            feedback.textContent = 'No puede superar 32 caracteres.';
            feedback.style.color = 'var(--danger)';
            return false;
        }
        feedback.textContent = 'Nombre valido.';
    ...
    async handleSaveProfile() {
        ...
        // MEJORA D: validacion en el cliente ANTES de llamar al backend (feedback inmediato, sin round-trip).
        if (!this._validateUsername()) {
            if (typeof UIAlerts !== 'undefined') {
                UIAlerts.showError('edit-profile-alert', 'Corrige el nombre de usuario antes de guardar.');
            }
            const input = document.getElementById('edit-username');
        ...
        // MEJORA D: spinner inmediato en el boton al iniciar el guardado.
        ...
        try {
            ...
            if (success) {
                ...
                // MEJORA D: cierre rapido (400ms) para que el usuario alcance a ver el mensaje de exito.
                setTimeout(() => {
                    this.closeModal();
                    const current = ProfileSystem.getCurrentProfile();
