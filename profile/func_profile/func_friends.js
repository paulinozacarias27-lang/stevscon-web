const FriendsSystem = {
    ...
    async sendRequest(targetUid) {
        ...
        if (!current) { this._showError('Debes iniciar sesion.'); return; }
    ...
    async acceptRequest(senderUid) {
        ...
        if (success) {
            this._showSuccess('Ahora son amigos.');
            ProfileSystem.openProfile(senderUid);
        } else {
    ...
    async rejectRequest(senderUid) {
        ...
        if (success) {
            this._showSuccess('Solicitud rechazada.');
            ProfileSystem.openProfile(senderUid);
        } else {
    ...
    confirmBlockUser(targetUid) {
        if (typeof UIAlerts !== 'undefined' && UIAlerts.showConfirm) {
            UIAlerts.showConfirm({
                ...
                message: '¿Seguro que quieres bloquear a este usuario? Se eliminara la amistad y no podra enviarte mensajes.',
    ...
    async renderFriendsPage() {
        if (typeof UIFriends === 'undefined' || !UIFriends.renderFriendsPage) {
            console.warn('UIFriends module not loaded.');
            return;
        }
        const current = ProfileSystem.getCurrentProfile();
        if (!current) return;
