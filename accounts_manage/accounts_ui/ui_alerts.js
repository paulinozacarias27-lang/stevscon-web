const UIAlerts = {
    ...
    // MEJORA J: showSuccess ahora se auto-oculta a los 5s con una animacion de desvanecido (clase success-auto-dismiss).
    showSuccess(containerId, message) {
        ...
        const escapedMessage = this._escape(message);
        container.innerHTML = `
            <div class="success-auto-dismiss" style="display: flex; align-items: center; gap: 8px; margin-bottom: 15px; padding: 10px 14px; background: rgba(139, 92, 246, 0.15); border: 1px solid var(--purple-accent); color: var(--text-main); border-radius: 8px; font-size: 0.9rem;">
                <i class="fa-solid fa-circle-check" style="color: var(--purple-accent);"></i>
                <span>${escapedMessage}</span>
            </div>
        `;

        // Limpiar cualquier temporizador previo para evitar cierres duplicados.
        if (this._successTimers && this._successTimers[containerId]) {
            clearTimeout(this._successTimers[containerId]);
        }
        this._successTimers = this._successTimers || {};
        this._successTimers[containerId] = setTimeout(() => {
            const box = container.querySelector('.success-auto-dismiss');
            if (box) {
    ...
    // MEJORA J: nueva alerta informativa (azul/neutra) para mensajes de estado, ej. "enviando...".
    showInfo(containerId, message) {
        const container = document.getElementById(containerId);
        if (!container) return;

        const escapedMessage = this._escape(message);
        container.innerHTML = `
            <div class="info-msg" style="display: flex; align-items: center; gap: 8px; margin-bottom: 15px; padding: 10px 14px; background: rgba(59, 130, 246, 0.15); border: 1px solid #3b82f6; color: var(--text-main); border-radius: 8px; font-size: 0.9rem;">
    ...
    clear(containerId) {
        ...
        if (this._successTimers && this._successTimers[containerId]) {
            clearTimeout(this._successTimers[containerId]);
            this._successTimers[containerId] = null;
        }
    },
